...
import { SOCIAL_LINKS } from "@/lib/constants/rodium-links";
...
export function LandingSocialLinks({
  ...
  return (
    <div className={`lp-social${className ? ` ${className}` : ""}`}>
      <a
      ...
      >
        <DiscordIcon className="lp-social-icon" />
      </a>
      <a
      ...
      >
        <YoutubeIcon className="lp-social-icon" />
      </a>
      <a
        href={SOCIAL_LINKS.x}
        target="_blank"
        ...
        aria-label={xLabel}
      >
        <Icon icon={X} className="lp-social-icon" size={16} />
      </a>
    </div>
  );
}

export function GithubMark({ className = "lp-social-icon" }: { className?: string }) {
